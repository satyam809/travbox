...
if($editresult['bookingType'] == 0 || !empty($editresult['roundTripId']))
{
...
Booking Id: <strong><?php echo 'TB/'.$year_range.'/F/'.encode($editresult['id']); ?></strong>  <br />
          ...
          Booking Time: <?php echo date('D, j M Y', strtotime($editresult['bookingDate'])); ?></td>
				<td width="50%" align="center" valign="center">
					...
					<?php 
          if(!empty($editresult['roundTripId']) && !empty($editresult['detailArray'])){
          ...
          while($flightwayres=mysqli_fetch_array($flightway)){
            ...
            <td colspan="2" align="center"><?php echo stripslashes($editresult['flightName']); ?></td>
            <td width="50%" align="center">
            <div style="font-size:18px; color:#000; text-transform:uppercase;"><?php echo $editresult['pnrNo']; ?></div>
    ...
    <?php 
     ...
     $flightway=GetPageRecord('*','flightBookingMaster',' roundTripId="'.$editresult['roundTripId'].'"  order by id asc '); 
      ...
      $flightway=GetPageRecord('*','flightBookingMaster',' id="'.$editresult['id'].'"  order by id asc '); 
    }
       while($flightwayres=mysqli_fetch_array($flightway)){ ?>
       ...
       <?php 
	   if(!empty($flightwayres['detailArray'])){ 
	   $data=(array) unserialize(stripslashes($flightwayres['detailArray']));
        $index=0;
        for($i=1; $i <= count($data['sI']); $i++){
     ...
     <?php $index++; } }else{ ?>
		...
		<table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="margin-bottom:10px;">
			 ...
			 <tr>
			   <td align="left" valign="top"><div style="font-size:14px; font-weight:500; color:#000000;"><?php echo $flightwayres['flightCode']; ?> -<?= $flightwayres['flightNo']; ?> </div>
				<?php echo $flightwayres['flightName']; ?></td>
				<td align="left" valign="top"><div style="font-size:14px; font-weight:500; color:#000000;">  <?php echo $flightwayres['pcc']; ?></td>
				<td align="left" valign="top"><strong><?php echo $flightwayres['fareClass']; ?></strong></td>
				...
				<td align="left" valign="top"><strong><?= $flightwayres['refundyes'] == 1 ? "Refundable" : "Non-Refundable"; ?></strong></td>
				   ...
				   <td align="left" valign="top">
				<div style="font-size:14px; font-weight:500; color:#000000;"><?php echo date('D, j M Y', strtotime($flightwayres['journeyDate'])); ?>, <?php echo $flightwayres['departureTime']; ?></div>
				<?php $fareType=explode('-',$flightwayres['source']); 
 				echo getflightdestination($fareType[1]); ?> <?= !empty($flightwayres['flightTerminal']) ? ',Terminal '.$flightwayres['flightTerminal']: '' ; ?></td>
				   <td align="left" valign="top">
				<div style="font-size:14px; font-weight:500; color:#000000;"><?php echo date('D, j M Y', strtotime($flightwayres['arrivalDate'])); ?>, <?php echo $flightwayres['arrivalTime']; ?></div>
				<?php $fareType=explode('-',$flightwayres['destination']);  echo getflightdestination($fareType[1]); ?></td>
				 <td align="left" valign="top"><?php 
				$diff = abs(strtotime($flightwayres['departureTime']) - strtotime($flightwayres['arrivalTime']));
				$tmins = $diff/60;
				$hours = floor($tmins/60);
				$mins = $tmins%60;
				 echo $hours . 'H , ' . $mins. 'M'; ?></td>
			 </tr>
      ...
      <table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000">
          <tr>
            ...
            <?php if($editresult['pcc']=="STUDENT" || $editresult['pcc']=="SENIOR_CITIZEN"){ ?>
            <td colspan="2" bgcolor="#E9E9E9"><strong>Dcument Id</strong></td>
          <?php } ?>
    ...
    //$totalcostings2=mysqli_fetch_array($rs5);
        ...
        ?>
          <tr>
            ...
            <?php if($editresult['pcc']=="STUDENT" || $editresult['pcc']=="SENIOR_CITIZEN"){ ?>
            <td><?php echo $paxData['documentId']; ?></td>
          <?php } ?>
  ...
  ?>
    ...
    <table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#CCCCCC">
      <tr>
       ...
       if(!empty($basic_fare)){
				$totalcost =  number_format(round($basic_fare+$basic_fare2));
			}else{
			 $totalcost = round($totalcostings['totalagentBaseFare']/$_REQUEST['tp']);
      ...
      $tex =  round($tax_Fare+$tax_Fare2);
        ...
        echo  $tex =  round((($totalcostings['totaltax']/$_REQUEST['tp'])/$_REQUEST['tp'])+($_REQUEST['markup']));
         }
         ?>
	  </td>
	...
	<table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#CCCCCC">
     ...
     <?php if($totalcostings['totalseatPrice'] > 0){ ?> <tr>
      ...
      <tr>
        <td width="25%"><strong>Total Fare </strong></td>
        <td colspan="2">Rs.<?php 
          echo number_format($totalcostings['totalagentBaseFare']+$totalcostings['totalagentFixedMakup']+($_REQUEST['markup']+$totalcostings['totalmealPrice']+$totalcostings['totalseatPrice']+$totalcostings['totalextraBaggagePrice']));            // echo number_format((($totalcostings['totalagentBaseFare']+$totalcostings['totalagentFixedMakup'])+
            // $totalcostings['totaltax']+$totalcostings['totalagentMarkup']+($_REQUEST['markup']) +
            // ($totalcostings['totalseatPrice'])+ ($totalcostings['totalmealPrice'])+
