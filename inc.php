...
//$hotelApiKey = '712228a05a6bda-c0a0-4e7b-8ee2-b0f5c70b8d16';// test key
$hotelApiKey  =  '610937160b61a20a-6bf6-42c9-814e-fe56eb1815af'; // live key
//$hotelApiKey = '';
$tripjackhotelurl='https://api.tripjack.com/';  // liv  tripjack

//$tripjackhotelurl='https://apitest.tripjack.com/';  // test tripjack
...
// if flight contains stops get other flight codes.
function nonStopFightId($rawjson,$seprator=',')
{
    $dd=unserialize(stripslashes($rawjson));
    $n=1;
    $flightCode = [];
    foreach((array) $dd['sI'] as $layoverFlight){
        array_push($flightCode,stripslashes($layoverFlight['fD']['aI']['code']) . '-' .stripslashes($layoverFlight['fD']['fN']));
    }

    // return implode(',',$flightCode);
    return implode($seprator,$flightCode);
}


// if flight logos.
function FightLogos($rawjson)
{
    $dd=unserialize(stripslashes($rawjson));
    if(!$imgurl || $imgurl == '')
    {
    ...
    $logs = '';
    foreach((array) $dd['sI'] as $layoverFlight){
        $logs .= '<img title="'.stripslashes(stripslashes($layoverFlight['fD']['aI']['name'])).'" style="margin: 2px 5px 0px 0px" src="'. $imgurl.getflightlogo(stripslashes(stripslashes($layoverFlight['fD']['aI']['name']))) .'" width="16" height="16">';
