...
<span onclick="loadpop('Add Hotel',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=adddomestichotelsmarkup" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Hotel</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-percent" aria-hidden="true"></i> Hotel Markup</span></div>
							...
							<thead>
								<tr>
								  <th width="2%"><div align="center">Sr.</div></th>
									<th width="40%">Name</th>
									<th width="35%"><div align="center">Markup</div></th>
...
if(mysqli_num_rows($a)>0){ while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">'.($roomlist['name']).' '.($roomlist['markupValue']).' '.($roomlist['markupType']).'</div>'; }  echo $roomslist; } ?> 
									...
									<td><div align="center">
									<button type="button" class="btn btn-primary btn-icon btn-sm"   onclick="loadpop('Add Markup',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addfaretypedomesticHotelMarkup&id=<?php echo encode($rest['id']); ?>"> <i class="fa fa-plus" aria-hidden="true"></i> Add Markup</button>
