...
<span onclick="loadpop('Add Flight',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=adddomesticflightsmarkup&typeid=<?php echo $_REQUEST['typeid']; ?>" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Flight</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><a href="display.html?ga=commissiontype" style="color:#000000;">Agent Type</a> - <span style="color:#e52b30; font-weight:800;"><?php echo stripslashes($editresult['name']); ?></span> - Flight Markup</span></div>


<div style="padding:10px; width:100%; border-bottom:1px solid #ddd;">
<table border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><input name="keyword" type="text" class="form-control" id="keyword" placeholder="Enter Keyword" value="<?php echo $_REQUEST['keyword']; ?>" style="width:200px;"><input name="ga" type="hidden" id="ga" value="<?php echo $_REQUEST['ga']; ?>" /><input name="typeid" type="hidden" id="typeid" value="<?php echo $_REQUEST['typeid']; ?>" /></td>
							...
							<thead>
								<tr>
								  <th width="2%"><div align="center">Sr.</div></th>
									...
									<th width="20%"><div align="center">Edit</div></th>
								</tr>
							</thead>
							<tbody>
...
$rs=GetRecordList('*','domesticFlightsMarkupMaster',' where parentId="'.$_SESSION['userid'].'" and agentTypeGroupId="'.decode($_REQUEST['typeid']).'" '.$search.' order by id desc  ','250',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								<tr>
								  <td width="2%"><div align="center"><?php echo $sNo; ?>.</div></td>
									<td width="20%"><div align="left"><strong><?php echo stripslashes($editresult['name']); ?></strong></div></td>
									<td width="20%"><div style="font-weight:500;"><?php echo stripslashes($rest['name']); ?></div>																		</td>
...
<?php
$roomslist='';
$a=GetPageRecord('*','fareTypedomesticFlightsMarkupMaster',' flightId="'.$rest['id'].'" and parentId="'.$_SESSION['userid'].'" order by id desc');
