...
<span  class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;"  onclick="loadpop('Add Vehicle Category',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addvehiclecategory">Add New</span>

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-list" aria-hidden="true"></i> Vehicle Category</span></div>
...
$rs=GetRecordList('*','vehicle_category',' '.$where.' ','25',$page,$targetpage);
...
<td><a <?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'vehicle_category')) !== false){ ?> class="dropdown-item" style="cursor:pointer; font-weight:600; background-color:transparent;" onclick="loadpop('Update Vehicle Category',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addvehiclecategory&id=<?php echo encode($rest['id']); ?>" <?php } ?>><?php echo stripslashes($rest['name']); ?></a></td>

	

	<td><?php echo stripslashes($rest['pax']); ?></td>
	...
	<?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'vehicle_category')) !== false){ ?>

	<a class="dropdown-item"  style="cursor:pointer;" onclick="loadpop('Edit Vehicle Category',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addvehiclecategory&id=<?php echo encode($rest['id']); ?>">
