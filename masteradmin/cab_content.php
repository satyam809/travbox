...
<span  class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;"  onclick="loadpop('Add Cab Content',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcabcontent">Add New</span>

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-list" aria-hidden="true"></i> Cab Display Content</span></div>
...
$rs=GetRecordList('*','cab_content',' '.$where.' ','25',$page,$targetpage);
...
<tr>
  ...
  <td><a class="dropdown-item" style="cursor:pointer; font-weight:600; background-color:transparent;" <?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'cab_content')) !== false){ ?> onclick="loadpop('Update Content',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcabcontent&id=<?php echo encode($rest['id']); ?>" <?php } ?>><?php echo stripslashes($rest['name']); ?></a></td>
	...
	<?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'cab_content')) !== false){ ?>

	<a class="dropdown-item"  style="cursor:pointer;" onclick="loadpop('Edit Content',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcabcontent&id=<?php echo encode($rest['id']); ?>">
