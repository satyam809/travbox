...
<span onclick="loadpop('Add Activity',this,'800px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addactivitylibrary" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Activity</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-blind" aria-hidden="true"></i> Activity</span>
</div>
...
$rs=GetRecordList('*','activityMaster',' where parentId="'.$LoginUserDetails['parentId'].'" '.$search.' order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								<tr>
									...
									<?php if($rest['sectionDuration']!=''){ ?><div style="font-weight:400; color:#666666; font-size:12px;"><i class="fa fa-clock-o" aria-hidden="true"></i> &nbsp;<?php echo stripslashes($rest['sectionDuration']); ?></div><?php } ?>																	</td>
									...
									<div style="margin-bottom:2px; cursor:pointer;" id="displayroomrate<?php echo ($rest['id']); ?>" onclick="loadpop('Add Activity Cost',this,'1300px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addactivitylibrarycost&id=<?php echo encode($rest['id']); ?>">
											<?php
											$roomslist='';
											$a=GetPageRecord('*','sys_ActivityCost',' activityId="'.$rest['id'].'" and validTo>="'.date('Y-m-d').'" and validFrom<="'.date('Y-m-d').'" and parentId="'.$LoginUserDetails['parentId'].'"  order by id desc');
											while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">Adult Cost: ('.$roomlist['adultCost'].' '.currencyname($roomlist['currencyId']).')'.'</div>'; }  echo $roomslist; ?> 
									</div>
									
									
									
									<div align="center"><span class="badge bg-blue" style="background-color:#2196f3; cursor:pointer;" onclick="loadpop('Add Activity Cost',this,'1300px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addactivitylibrarycost&id=<?php echo encode($rest['id']); ?>"><i class="fa fa-plus" aria-hidden="true"></i> Add</span></div></td>
									...
									<td><div align="center"><a  style="cursor:pointer;" onclick="loadpop('Edit Activity',this,'800px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addactivitylibrary&id=<?php echo encode($rest['id']); ?>">
