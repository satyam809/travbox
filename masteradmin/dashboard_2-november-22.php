...
<div class="page-content pt-0">  
		...
		<div style="text-align:center; font-size:14px;"> Pending Flights </div>
			...
			<div class="col-xl-2">
			<div class="card">
			<div class="card-body" style="text-align:center;background-color:#fffdb3;padding: 12px;">
		
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td colspan="2" align="center" style="padding: 11px 0px;"><strong>API Balance</strong></td>
    </tr>
  <tr>
    <td align="center"><h2 style="line-height:22px;"><strong id="loadapibalance" style="color:#CC0000; font-size:20px;line-height: 20px; ">0</strong></h2>
		
		<script>
				 $('#loadapibalance').load('loadapibalance.php');
				 </script><div style="text-align:center; font-size:12px;"><strong>Kafila</strong></div></td>
    <td width="50%" align="center"><h2 style="line-height:22px;"><strong id="loadapibalance" style="color:#CC0000; font-size:20px;line-height: 20px;  ">0</strong></h2>
		
	 <div style="text-align:center; font-size:12px;"><strong>TBO</strong></div></td>
  </tr>
</table>
		...
		<h2><strong><?php
				$a=GetPageRecord('count(id) as totalhoteleq','hotelEnquiry',' 1 '); 
				$gettotal=mysqli_fetch_array($a); 
				echo $gettotal['totalhoteleq'];
				?></strong></h2>
		
		<div style="text-align:center; font-size:14px;"> Hotel Enquiry </div>
			</div>
			</div>
			</div>
			<div class="col-xl-2">
		...
		<h2><strong><?php
				$a=GetPageRecord('count(id) as totalpackageeq','packageEnquiry',' 1 '); 
				$gettotal=mysqli_fetch_array($a); 
...
{
  country: "Expenses",
  value: <?php $ad=GetPageRecord('SUM(totalFare) as totalagentcost','flightBookingMaster',' status=2 ');
...
{
  country: "Sale",
  value: <?php $ad=GetPageRecord('SUM(agentTotalFare) as totalagentcost','flightBookingMaster',' status=2 ');
$restflightsamount=mysqli_fetch_array($ad); echo $totalmarkup=$restflightsamount['totalagentcost']; ?>
},

{
  country: "Commission",
  value: <?php $ad=GetPageRecord('SUM(agentCommision) as totalagentCommision','flightBookingMaster',' status=2 ');
$restflightsamount=mysqli_fetch_array($ad); echo $totalcommission=$restflightsamount['totalagentCommision']; ?>
},

{
  country: "Earn",
  value: <?php $ad=GetPageRecord('SUM(agentMarkup) as totalagentcost','flightBookingMaster',' status=2 ');
