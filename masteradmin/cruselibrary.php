...
<span onclick="loadpop('Add Cruise',this,'800px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcruselibrary" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Cruise</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-ship" aria-hidden="true"></i> Cruise / Ferry</span>
</div>
...
$rs=GetRecordList('*','cruseMaster',' where parentId="'.$LoginUserDetails['parentId'].'" '.$search.' order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								<tr>
									...
									<div style="margin-bottom:2px; cursor:pointer;" id="displayroomrate<?php echo ($rest['id']); ?>" onclick="loadpop('Add Cruise Cost',this,'1300px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcruselibrarycost&id=<?php echo encode($rest['id']); ?>">
											 <?php
											$roomslist='';
											$a=GetPageRecord('*','sys_CruseCost',' cruseId="'.$rest['id'].'" and validTo>="'.date('Y-m-d').'" and validFrom<="'.date('Y-m-d').'" and parentId="'.$LoginUserDetails['parentId'].'" group by seatId order by id desc');
											while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">'.cruiseSeatName($roomlist['seatId']).' ('.$roomlist['adultCost'].' '.currencyname($roomlist['currencyId']).')'.'</div>'; }  echo $roomslist; ?> 
									</div>
									
									
									
									<div align="center"><span class="badge bg-blue" style="background-color:#2196f3; cursor:pointer;" onclick="loadpop('Add Cruise Cost',this,'1300px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcruselibrarycost&id=<?php echo encode($rest['id']); ?>"><i class="fa fa-plus" aria-hidden="true"></i> Add</span></div></td>
									...
									<td><div align="center"><a  style="cursor:pointer;" onclick="loadpop('Edit Cruise',this,'800px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcruselibrary&id=<?php echo encode($rest['id']); ?>">
