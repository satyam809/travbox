...
<span onclick="loadpop('Add Flight',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=adddomesticflightscommission" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Flight</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-percent" aria-hidden="true"></i> Agent Commission</span></div>
							...
							<thead>
								<tr>
								  ...
								  <th width="25%"><div align="center">Fare Type</div></th>
									<th width="25%" style=" display:none;">Cash Back </th>
...
?>
								<tr>
								  ...
								  <td width="25%"><div style="font-weight:500;"><?php echo stripslashes($rest['name']); ?></div>																		</td>
									...
									<div style="margin-bottom:2px; cursor:pointer;" id="displayroomrate<?php echo ($rest['id']); ?>" onclick="loadpop('Add Fare Type Commission',this,'900px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addfaretypedomesticFlightscommission&id=<?php echo encode($rest['id']); ?>">
<?php
$roomslist='';
$a=GetPageRecord('*','fareTypedomesticFlightsCommissionMaster',' flightId="'.$rest['id'].'" and parentId="'.$LoginUserDetails['parentId'].'" order by id desc');
if(mysqli_num_rows($a)>0){ while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">'.($roomlist['name']).' '.($roomlist['markupValue']).' '.($roomlist['markupType']).'</div>'; }  echo $roomslist; } ?> 
									...
									<td width="25%"  style=" display:none;"><div style="margin-bottom:2px; cursor:pointer;" id="displayroomrate<?php echo ($rest['id']); ?>" onclick="loadpop('Add Fare Type Commission',this,'900px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addfaretypedomesticFlightscommission&id=<?php echo encode($rest['id']); ?>">
<?php
$roomslist2='';
$ab=GetPageRecord('*','fareTypedomesticFlightsCommissionMaster',' flightId="'.$rest['id'].'" and parentId="'.$LoginUserDetails['parentId'].'" order by id desc');
if(mysqli_num_rows($ab)>0){ while($roomlistcb=mysqli_fetch_array($ab)){ $roomslist2.='<div class="roomratelist">'.($roomlistcb['name']).' '.($roomlistcb['cashBackValue']).' '.($roomlistcb['cashBackType']).'</div>'; }  echo $roomslist2; } ?> 
									...
									<button type="button" class="btn btn-primary btn-icon btn-sm"   onclick="loadpop('Add Fare Type Commission',this,'900px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addfaretypedomesticFlightscommission&id=<?php echo encode($rest['id']); ?>"> <i class="fa fa-plus" aria-hidden="true"></i> Add Fare Type </button>
									
									
									<a  style="cursor:pointer;" onclick="loadpop('Edit Flight',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=adddomesticflightscommission&id=<?php echo encode($rest['id']); ?>">
