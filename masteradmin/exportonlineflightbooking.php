...
$filename = 'online_flight_booking_'.mt_rand().'.xls';
...
$query=GetPageRecord('*','flightBookingMaster','1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and agentBookingType=0 and bookingType=0 and status!=0 '.$search.' order by id desc'); 
...
$status='<span class="badge bg-blue" style="background-color:#f9392f;">Rejected</span>';
		...
		$data .= '<tr>
					...
					<td>'.strip($agentData['phone']).'</td>
					<td>'.strip($agentData['email']).'</td>
					<td>'.$trip.'</td>
