...
if($_REQUEST['dltid']!=''){ 
deleteRecord('agent_fareTypedomesticFlightsMarkupMaster','id="'.decode($_REQUEST['dltid']).'" and agentId="'.decode($_REQUEST['agentId']).'"');  
}
?>
...
$n=1;  
$roomslist='';
 $rs=GetPageRecord('*','agent_fareTypedomesticFlightsMarkupMaster',' flightId="'.$id.'" and agentId="'.decode($_REQUEST['agentId']).'" order by id desc');
...
<?php
$roomslist='';
$a=GetPageRecord('*','agent_fareTypedomesticFlightsMarkupMaster',' flightId="'.$id.'" and agentId="'.decode($_REQUEST['agentId']).'" order by id desc');
