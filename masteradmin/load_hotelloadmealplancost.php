...
if($_REQUEST['dltid']!=''){ 
deleteRecord('sys_HotelMealPlanCost','id="'.decode($_REQUEST['dltid']).'" and parentId="'.$LoginUserDetails['parentId'].'"'); 
}
?>
...
$n=1;  
$roomslist='';
$rs=GetPageRecord('*','sys_HotelMealPlanCost',' hotelId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" order by id desc');
...
<td align="left"><?php echo gethotelmealplan($rest['mealPlanId']); ?></td>
...
<?php
$roomslist='';
$a=GetPageRecord('*','sys_HotelMealPlanCost',' hotelId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" and validTo>="'.date('Y-m-d').'" and validFrom<="'.date('Y-m-d').'" group by mealPlanId order by id desc');
while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">'.gethotelmealplan($roomlist['mealPlanId']).' ('.$roomlist['adultCost'].' '.currencyname($roomlist['currencyId']).')'.'</div>'; }   ?> 			
						
<script>
$('#displaymealrate<?php echo $id; ?>').html('<?php echo $roomslist; ?>');
</script>						
							</tbody>
						</table>

<?php if($n==1){ ?>
<div style="font-size:14px; color:#999999; text-align:center; padding:20px 0px; width:100%;">No Meal Plan Found</div>
<?php } ?>