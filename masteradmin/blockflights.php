...
<span onclick="loadpop('Add Block Flights',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addblockflights&typeid=<?php echo $_REQUEST['typeid']; ?>" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Block Flights</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><a href="display.html?ga=commissiontype" style="color:#000000;">Agent Type</a> - <span style="color:#e52b30; font-weight:800;"><?php echo stripslashes($editresult['name']); ?></span> - <i class="fa fa-ban" aria-hidden="true"></i> Block Flights</span></div>
...
$rs=GetRecordList('*','blockFlightMaster',' where parentId="'.$_SESSION['userid'].'"  and agentTypeGroupId="'.decode($_REQUEST['typeid']).'" '.$search.' order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
								...
								<tr>
								  ...
								  <td width="15%"><div style="font-weight:500;"><?php echo stripslashes($rest['name']); ?></div>																		</td>
									...
									<div style="margin-bottom:2px; cursor:pointer;" id="displayroomrate<?php echo ($rest['id']); ?>" >
											<?php
											$roomslist='';
											$a=GetPageRecord('*','fareTypeblockFlightMaster',' blockFlightId="'.$rest['id'].'" and parentId="'.$_SESSION['userid'].'" order by id desc');
