...
if($_REQUEST['dltid']!=''){ 
deleteRecord('sys_HotelExtraCost','id="'.decode($_REQUEST['dltid']).'" and parentId="'.$LoginUserDetails['parentId'].'"'); 
}
?>
...
$n=1;  
$roomslist='';
$rs=GetPageRecord('*','sys_HotelExtraCost',' hotelId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" order by id desc');
...
<td align="left"><?php echo gethotelextra($rest['extraId']); ?></td>
...
<?php
$roomslist=''; 
$a=GetPageRecord('*','sys_HotelExtraCost',' hotelId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" and validTo>="'.date('Y-m-d').'" and validFrom<="'.date('Y-m-d').'" group by extraId order by id desc');
while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">'.gethotelextra($roomlist['extraId']).' ('.$roomlist['adultCost'].' '.currencyname($roomlist['currencyId']).')'.'</div>'; }   ?> 			
						
<script>
$('#displayextrarate<?php echo $id; ?>').html('<?php echo $roomslist; ?>');
</script>						
							</tbody>
						</table>
