...
<?php 
$rs=GetPageRecord('*','activityMaster','cityId="'.$_REQUEST['pickupCityfromCity'].'"   and (name like "%'.trim(strip($_REQUEST['keyword'])).'%") order by name asc limit 0,10'); 
...
<?php } ?>
 ...
 if(editid!=''){ 
 var popaction = $('#activityaddnewbtn'+editid).attr('popaction'); 
 $('#popcontent').load('loadpopup.php?'+popaction+'&filledsightseeingid='+id);
 
 } else { 
  var popaction = $('#activityaddnewbtn').attr('popaction'); 
  $('#popcontent').load('loadpopup.php?'+popaction+'&filledsightseeingid='+id);
 }
 }
 </script>