...
$a=GetPageRecord('name','vehicle_category','id="'.$id.'"'); 
...
<span  class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;"  onclick="loadpop('Add Vehicle',this,'800px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcabvehicle">Add New</span>

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-list" aria-hidden="true"></i> Cab Vehicle</span></div>
...
$rs=GetRecordList('*','vehiclemaster',' '.$where.' ','25',$page,$targetpage);
...
<tr>
  ...
  <td><?php if($rest['photo']!=''){ ?><img src="<?php echo $fullurl; ?>upload/<?php echo stripslashes($rest['photo']); ?>" width="100" /><?php } ?></td>



<td><a class="dropdown-item" style="cursor:pointer; font-weight:600; background-color:transparent;" <?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'vehicle')) !== false){ ?> onclick="loadpop('Update Vehicle',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addvehicle&id=<?php echo encode($rest['id']); ?>" <?php } ?>><?php echo stripslashes($rest['name']); ?></a></td>

  

  <td><?php echo getCategoryName($rest['category_id']); ?></td>

  

  <td><?php if($rest['fuel']==1){ ?><span class="badge badge-success">Diesel</span><?php }if($rest['fuel']==2){ ?><span class="badge badge-success">Petrol</span><?php }if($rest['fuel']==2){ ?><span class="badge badge-success">CNG</span><?php } ?></td>
	...
	<?php if ($LoginUserDetails["id"]==1 || (strpos($LoginUserDetails["permissionAddEdit"], 'vehicle')) !== false){ ?>

	<a class="dropdown-item"  style="cursor:pointer;" onclick="loadpop('Edit Vehicle',this,'600px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addcabvehicle&id=<?php echo encode($rest['id']); ?>"><button type="button" class="btn btn-secondary btn-sm" >Edit</button></a>
