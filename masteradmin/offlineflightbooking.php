...
<div class="page-content pt-0" > 
  ...
  <tr>
    <td><a href="display.html?ga=offlineflightbooking&stage=&fromdate=&todate=&status=2&keyword="><div style="background-color: #46cd93; height: 30px; color: #fff; border-radius: 30px; text-align: center; line-height: 30px; font-size: 16px; font-weight: 600; width: 80px; "><?php
				$a=GetPageRecord('count(id) as totalflight','flightBookingMaster','1 and agentOffline=1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and status=2'); 
				$gettotal=mysqli_fetch_array($a); 
				echo $gettotal['totalflight'];
				?></div></a></td>
    <td style="padding-left:0px;">Confirmed</td>
    <td><a href="display.html?ga=offlineflightbooking&stage=&fromdate=&todate=&status=3&keyword="><div style="background-color: #f9392f; height: 30px; color: #fff; border-radius: 30px; text-align: center; line-height: 30px; font-size: 16px; font-weight: 600; width: 80px;  "><?php
				$a=GetPageRecord('count(id) as totalflight','flightBookingMaster','1 and agentOffline=1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and status=3'); 
				$gettotal=mysqli_fetch_array($a); 
				echo $gettotal['totalflight'];
				?></div></a></td>
    <td style="padding-left:0px;">Cancel</td>
	
	<td><a href="display.html?ga=offlineflightbooking&stage=&fromdate=&todate=&status=4&keyword="><div style="background-color: #f9392f; height: 30px; color: #fff; border-radius: 30px; text-align: center; line-height: 30px; font-size: 16px; font-weight: 600; width: 80px;  "><?php
				$a=GetPageRecord('count(id) as totalflight','flightBookingMaster','1 and agentOffline=1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and status=4 '); 
				$gettotal=mysqli_fetch_array($a); 
				echo $gettotal['totalflight'];
				?></div></a></td>
    ...
    <td>
	<a href="display.html?ga=offlineflightbooking&stage=&fromdate=&todate=&status=1&keyword="><div style="background-color: #FF6600; height: 30px; color: #fff; border-radius: 30px; text-align: center; line-height: 30px; font-size: 16px; font-weight: 600; width: 80px;  "><?php
				$a=GetPageRecord('count(id) as totalflight','flightBookingMaster','1 and agentOffline=1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and (status=1 or status=0)'); 
				$gettotal=mysqli_fetch_array($a); 
				echo $gettotal['totalflight'];
				?></div></a>	</td>
...
$rs=GetRecordList('*','flightBookingMaster',' where 1 and agentOffline=1 and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'") and agentBookingType=0 and bookingType=0 '.$search.'  order by id desc  ','25',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width:200px; margin-top:2px; color:#3366CC;">Comm.: <strong>&#8377;<?php echo ($rest['agentCommision']); ?></strong></div>
    ...
    <td width="5%"><div class="btn-group" role="group" aria-label="Option">
  ...
  <a href="<?php echo $fullurl; ?>flightInvoice.php?id=<?php echo encode($rest['id']); ?>" target="_blank"><button type="button" class="btn btn-secondary"><i class="fa fa-info" aria-hidden="true"></i></button></a>
  	<?php } ?>
<a onclick="loadpop('Update PNR',this,'700px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=updatePNR&id=<?php echo encode($rest['id']); ?>"><button type="button" class="btn btn-secondary"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>
