...
<span onclick="loadpop('Add Hotel Category',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addhotelcategorylibrary" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Hotel Category</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-list" aria-hidden="true"></i> Hotel Category</span>
</div>
...
$rs=GetRecordList('*','sys_hotelCategory',' where parentId="'.$LoginUserDetails['parentId'].'" '.$search.' order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								<tr>
									...
									<td width="10%"><div align="center"><a  style="cursor:pointer;" onclick="loadpop('Edit Hotel Category',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addhotelcategorylibrary&id=<?php echo encode($rest['id']); ?>">
