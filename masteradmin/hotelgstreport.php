 ...
 <div class="page-header" style="margin-top: 46px;">
		...
		<div class="page-header-content header-elements-md-inline">
			<div class="page-title d-flex">
				<h4><a href="<?php echo $fullurl; ?>"><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Dashboard</span></a> - Hotel GST Report </h4> 
			</div>
...
<option value="<?php echo $rest['id']; ?>" <?php if($_REQUEST['agent']==$rest['id']){ ?>selected="selected"<?php } ?>><?php echo stripslashes($rest['companyName']); ?></option>    
		...
		<div class="col-xl-3">
			...
			<a href="export_hotelgstreport.php?fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&agent=<?php echo $_REQUEST['agent']; ?>&keyword=<?php echo $_REQUEST['keyword']; ?>" target="_blank"><button class="btn btn-light bg-primary border-primary text-white" type="button" style="margin-left:5px;">Export</button></a>
			</span>
			</div>
		  </div>
...
<div class="page-content pt-0" > 
		...
		<div class="content-wrapper">
			...
			<div class="card-header header-elements-inline">
						<h5 class="card-title">Hotel  GST Report List</h5>
...
$rs=GetRecordList('*','sys_balanceSheet',' where 1 and amount>0 and bookingId in (select BookingNumber from hotelBookingMaster where status=2) and agentId in (select id from sys_userMaster where parentId="'.$_SESSION['userid'].'")  and billType="hotel" group by bookingId  order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								...
								<tr>
								  ...
								  <td align="left" valign="top"><a href="<?php echo $fullurl; ?>hotelinvoice_display.php?id=<?php echo encode($bookingdata['id']); ?>" target="_blank"><?php echo encode($bookingdata['id']); ?></a></td>
