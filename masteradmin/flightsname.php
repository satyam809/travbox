...
<span onclick="loadpop('Add Room Type',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addflightname" class="badge bg-blue dropdown-toggle caret-0" style="background-color: #0cb5b5; position: absolute; right: 15px; top: 13px; font-size: 12px; cursor: pointer;" >Add Flight Logo</span> 

<span class="text-muted" style="font-weight:500; color:#000000 !important;"><i class="fa fa-plane" aria-hidden="true"></i> Flight Logo</span>
</div>
...
<table class="table">
							<thead>
								<tr>
								  <th width="1%"style="padding-left:20px !important;">Logo</th>
...
$rs=GetRecordList('*','sys_flightName',' where parentId="'.$LoginUserDetails['parentId'].'" '.$search.' order by id desc  ','100',$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2];  
...
?>
								<tr>
								  <td width="1%" style="padding-left:20px !important;"><div style="width:50px; height:50px; overflow:hidden; border:1px solid #ddd; "><img src="<?php if($rest['details']!=''){ echo 'upload/'.stripslashes($rest['details']); } else { ?>assets/hotelnoimage.png<?php } ?>" style="width:50px; height:50px;" /></div></td>
									...
									<?php if($rest['editDate']!='' && $rest['editDate']!='0'){ echo date('d-m-Y h:i A',strtotime($rest['editDate'])); } else { echo '-'; } ?></div></td>
									
									<td width="1%"><div align="center"><a  style="cursor:pointer;" onclick="loadpop('Edit Room Type',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addflightname&id=<?php echo encode($rest['id']); ?>">
