...
<div class="page-content pt-0"> 
  ...
  <tr>
    <td align="left"><h5 class="card-title" ><?php echo $editresult['companyName']; ?> - Balance Sheet<div style="font-size:14px;"><strong>Remaining Balance:&nbsp;&#8377;<span style="color:#FF0000;"><?php echo round($totalwalletBalance); ?></span></strong></div>
    </h5>
	</td>
    <td align="right"><button type="button" class="btn btn-primary btn-sm" onclick="loadpop('Add Payment',this,'500px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=addNewTransaction&agentId=<?php echo $_REQUEST['id']; ?>">Add Payment</button></td>
  </tr>
</table>
...
<tr>
	...
	<?php } ?>
	<?php if($agentWebsitePages['bookingType']=='flight_commision' || $agentWebsitePages['bookingType']=='hotel_commision' || $agentWebsitePages['bookingType']=='flight_GST' || $agentWebsitePages['bookingType']=='TDS' || $agentWebsitePages['bookingType']=='hotelTDS'  || $agentWebsitePages['bookingType']=='Facilitating' || $agentWebsitePages['bookingType']=='Markup' || $agentWebsitePages['bookingType']=='hotelMarkup' || $agentWebsitePages['bookingType']=='Facilitating_GST' || $agentWebsitePages['bookingType']=='RejectBankRefund'){ ?>
...
<?php if($agentWebsitePages['remarks']!=''){ ?><div style="font-size:12px; color:#666666;"><?php echo $agentWebsitePages['remarks']; ?></div><?php } ?>
	...
	<td align="right" valign="top"><div align="right"><?php echo strip($balance); ?> INR</div></td>
					...
					<div style="display:none;" id="divaction"></div>
					
					<script>
						function deletenewsevent(id){
							if(confirm('Are you sure want to delete?')){
								$('#divaction').load('actionpage.php?action=deletenewsevent&agentId=<?php echo $_REQUEST['id']; ?>&deleteId='+id);
							}
						}
					</script> 
