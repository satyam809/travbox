...
if($_REQUEST['dltid']!=''){ 
deleteRecord('sys_ActivityCost','id="'.decode($_REQUEST['dltid']).'" and parentId="'.$LoginUserDetails['parentId'].'"'); 
}
?>
...
$n=1;  
$roomslist='';
$rs=GetPageRecord('*','sys_ActivityCost',' activityId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" order by id desc');
...
<td align="left"><?php echo getactivityname($rest['activityId']); ?></td>
...
<?php
$roomslist='';
$a=GetPageRecord('*','sys_ActivityCost',' activityId="'.$id.'" and parentId="'.$LoginUserDetails['parentId'].'" and validTo>="'.date('Y-m-d').'" and validFrom<="'.date('Y-m-d').'"   order by id desc');
while($roomlist=mysqli_fetch_array($a)){ $roomslist.='<div class="roomratelist">Adult Cost: ('.$roomlist['adultCost'].' '.currencyname($roomlist['currencyId']).')'.'</div>'; }   ?> 			
