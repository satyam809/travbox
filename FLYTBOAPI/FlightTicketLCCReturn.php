...
$infant = $data['infant'];
 ...
 $fare_quote_results=$fare_quote_result['Response']['Results'];
$FareBreakdown = $fare_quote_results['FareBreakdown']; 

   foreach($FareBreakdown as $fbse)
{
    $PassengerType=$fbse['PassengerType'];
	if($PassengerType == '1')
	{
	   $PassengerCount=$fbse['PassengerCount'];
	   $currencyadlt =  $fbse['Currency'];
	   
	   $BaseFareadlts = $fbse['BaseFare']; 
	   $BaseFareadlt= $BaseFareadlts / $PassengerCount;
	   
	   $Taxadlts = $fbse['Tax']; 
	   $Taxadlt= $Taxadlts / $PassengerCount;

	   $YQTaxadlts = $fbse['YQTax']; 
	   $YQTaxadlt = $YQTaxadlts / $PassengerCount;
	   
	   $AdditionalTxnFeeOfrdadlt = $fbse['AdditionalTxnFeeOfrd']; 
	   $AdditionalTxnFeePubadlt = $fbse['AdditionalTxnFeePub']; 
	   $PGChargeadlt = $fbse['PGCharge']; 
	   $AirTransFeeadlt = "0"; 
	    
	} 
	if($PassengerType == '2')
	{
	   $PassengerCountchld=$fbse['PassengerCount'];
	   $currencychld =  $fbse['Currency'];
	   
	   $BaseFarechlds = $fbse['BaseFare']; 
	   $BaseFarechld= $BaseFarechlds / $PassengerCountchld;
	   $Taxchlds = $fbse['Tax']; 
	   $Taxchld = $Taxchlds / $PassengerCountchld;
	   
	   $YQTaxchlds = $fbse['YQTax']; 
	   $YQTaxchld = $YQTaxchlds / $PassengerCountchld;
	   
	   $AdditionalTxnFeeOfrdchld = $fbse['AdditionalTxnFeeOfrd']; 
	   $AdditionalTxnFeePubchld = $fbse['AdditionalTxnFeePub']; 
	   $PGChargechld = $fbse['PGCharge']; 
	   $AirTransFeechld = "0";
	} 
	if($PassengerType == '3')
	{
	   $PassengerCountinfnt=$fbse['PassengerCount'];
	   $currencyinfnt =  $fbse['Currency'];
	   
	   $BaseFareinfnts = $fbse['BaseFare']; 
	   $BaseFareinfnt = $BaseFareinfnts / $PassengerCountinfnt;
	   $Taxinfnts = $fbse['Tax']; 
	   $Taxinfnt = $Taxinfnts / $PassengerCountinfnt;
	   
	   $YQTaxinfnts = $fbse['YQTax']; 
	   $YQTaxinfnt = $YQTaxinfnts / $PassengerCountinfnt;
	   
	   $AdditionalTxnFeeOfrdinfnt = $fbse['AdditionalTxnFeeOfrd']; 
	   $AdditionalTxnFeePubinfnt = $fbse['AdditionalTxnFeePub']; 
...
$lpsn=1;$i=0;
 while($adult > 0 ) {
    	...
    	// $ht_psng['DateOfBirth'] = "2000-12-06T00:00:00";
		 ...
		 $DateOfBirth = date("Y-m-d", strtotime($pass[$i]['dob']));
		 $DateOfBirth= $DateOfBirth.'T00:00:00';
    	 $ht_psng['DateOfBirth'] = $DateOfBirth;
    	 
		 $ht_psng['Gender'] = "1";
		 $ht_psng['PassportNo'] = $pass[$i]['passport_no'];
		 
		 $PassportExpiry = date("Y-m-d", strtotime($pass[$i]['passport_exp']));
		 $PassportExpiry= $PassportExpiry.'T00:00:00';
		 $ht_psng['PassportExpiry'] = $PassportExpiry;
	    ...
	    ########################### SEAT ########################## 
    		...
    		// print_r($seatval); exit;
    		 foreach($seatref as $seatrefs){ 
	    	  ...
	    	  $SeatDynamic= array();
    	      $ht_psngst['AirlineCode'] = $adtstAirlineCode;
              $ht_psngst['FlightNumber'] = $adtstFlightNumber;
              $ht_psngst['CraftType'] = $adtstCraftType;
              $ht_psngst['Origin'] = $adtstOrigin;
              $ht_psngst['Destination'] = $adtstDestination;
              $ht_psngst['AvailablityType'] = $adtstAvailablityType;
              $ht_psngst['Description'] = $adtstDescription;
              $ht_psngst['Code'] = $adtstCode;
              $ht_psngst['RowNo'] = $adtstRowNo;
              $ht_psngst['SeatNo'] = $adtstSeatNo;
              $ht_psngst['SeatType'] = $adtstSeatType;
              $ht_psngst['SeatWayType'] = $adtstSeatWayType;
              $ht_psngst['Compartment'] = $adtstCompartment;
              $ht_psngst['Deck'] = $adtstDeck;
              $ht_psngst['Currency'] = $adtstCurrency;
              $ht_psngst['Price'] = $adtstPrice;
              
    	          $SeatDynamic[]= $ht_psngst;
	    	      $ht_psng['SeatDynamic'] =$SeatDynamic;
    		 }
...
$lpsn11=1;$j=0;
 while($child > 0 ) { 		
		 ...
		 $DateOfBirth = date("Y-m-d", strtotime($passs[$j]['dob']));
		 $DateOfBirth= $DateOfBirth.'T00:00:00';
    	 $ht_psng2['DateOfBirth'] = $DateOfBirth;
    	 
		 $ht_psng2['Gender'] = "1";
		 $ht_psng2['PassportNo'] = $passs[$j]['passport_no'];
		 
		 $PassportExpiry = date("Y-m-d", strtotime($passs[$j]['passport_exp']));
		 $PassportExpiry= $PassportExpiry.'T00:00:00';
		 $ht_psng2['PassportExpiry'] = $PassportExpiry;
...
$lpsn111=1;$k=0;
 while($infant > 0 ) { 		
		 ...
		 $ht_psng3['Gender'] = "1";
		 $ht_psng3['PassportNo'] = $pass_infant[$k]['passport_no'];
		 
		 $PassportExpiry = date("Y-m-d", strtotime($pass_infant[$k]['passport_exp']));
		 $PassportExpiry= $PassportExpiry.'T00:00:00';
		 $ht_psng3['PassportExpiry'] = $PassportExpiry;
...
$req=file_put_contents("FLYTBOJSON/FlyShopTicketLCCReturnReq.txt","$req");
  
$postdata = file_get_contents("FLYTBOJSON/FlyShopTicketLCCReturnReq.txt","$req"); //Take JSON input from Postman Client
//echo $postdata;
...
$results=file_put_contents("FLYTBOJSON/FlyShopTicketLCCReturnRes.txt","$flightRes");
...
//echo '<pre>';print nl2br(print_r($ticket_result_LCC, true));echo '</pre>'; 
?>