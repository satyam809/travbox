...
// insert query
		...
		$i = 1;
			...
			$getCalCost=calculateflightcost(encode($agentid),$airline,$flightType,$ResultFareType,$toalPax,$totalPaxFare,$totalTax);
  ...
  $namevalue ='UID="",TID="",ResultIndex="'.$ResultIndex.'",ORG_CODE="'.$dep_code.'",ORG_NAME="'.$depcityy.'",DES_CODE="'.$arr_codee.'",DES_NAME="'.$arrcityy.'",DEP_DATE="'.$msdate2['0'].'",DEP_TIME="'.createTimeArrDept($msdate2['1']).'",ARRV_DATE="'.$msdate1['0'].'",ARRV_TIME="'.createTimeArrDept($msdate1['1']).'",FLIGHT_CODE="'.trim($airlinecode).'",FLIGHT_NAME="'.$airline.'",FLIGHT_NO="'.trim($fno).'",FARE_TYPE="'.$ResultFareType.'",SEAT="'.$NoOfSeatAvailable.'",STOP="'.$flight_segment.'",AMT="'.$PublishedFare.'",DISPLAY_AMT="'.$PublishedFare.'",DUR="'.$hours.'H '.$minutes.'M'.'",S_CODE="'.$S_CODE.'",CN_CODE="'.$CN_CODE.'",OI="",PCC="'.$ResultFareType.'",TAX_BREAKUP="0",FLIGHT_INFO="'.$FLIGHT_INFO.'",NET_FARE="'.$OfferedFare.'",refundyes="'.$refstatuss.'",AirlineRemark="",F_CLASS="'.$PC.'",SECTOR="",adfare="'.$adfare.'",agfare="'.$agfare.'",clfare="'.$clfare.'",CON_DETAILS="",PARAM_DATA="",agentId="'.$_SESSION['agentUserid'].'",searchJson="",tripType=1,acom=""';
...
$todestexp = explode('-',$toDestinationFlight);
  ...
  "Segments": [
    {
      ...
      "Destination": "'.$todestexp[0].'",
      "FlightCabinClass": "2",
      "PreferredDepartureTime": "'.date('Y-m-d',strtotime('+ 1 days')).'T00:00:00'.'",
 ...
 $ch = curl_init();
curl_setopt($ch, CURLOPT_URL,'http://api.tektravels.com/BookingEngineService_Air/AirService.svc/rest/GetCalendarFare/');
