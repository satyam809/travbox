...
$fare_rule_ib_result=array();
try
{
$req=str_replace('\/','/',json_encode($opta));
$req=file_put_contents("FLYTBOJSON/FareRuleIBReq.txt","$req");
  
$postdata = file_get_contents("FLYTBOJSON/FareRuleIBReq.txt","$req"); //Take JSON input from Postman Client
//echo $postdata;
...
$results=file_put_contents("FLYTBOJSON/FareRuleIBRes.txt","$flightRes");
$fare_rule_ib_result = json_decode($flightRes,true);
}
catch(Exception $e)
{
...
// echo '<pre>';print nl2br(print_r($fare_rule_ib_result, true));echo '</pre>'; exit;
?>