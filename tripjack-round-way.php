...
$jsonData = rtrim($flightRes, "\0"); 
$search_result=json_decode($jsonData,true);
...
if($search_result['status']['success']==true)
{
	...
	if(count($flightInfoResultOnward)>0)
	{
	 ...
	 $commissiondeff=round(($totalPaxFare+$totalTax)-$OfferedFare);
			...
			$duration=intdiv($durationmain, 60).'H :'. ($durationmain % 60).' M'; 
				...
				$agentFixedMakup=round(agentfixmarkup(encode($agentid),$airline,$flightType,$FareBasisCode,$toalPax,$getCalCost2[1],$totalTax));
  ...
  // Return Flight 
	...
	if(count($flightInfoResultReturn)>0)
	{
		...
		{
			$searchJson=$flightInfoResultReturnValue;
			// get Source Information
			 ...
			 $ORG_city=$flightInfoResultReturnValue['sI'][0]['da']['city'];
			  ...
			  $departureDateArr=explode('T',$departureDate);
			  $departurTime=$departureDateArr[1];
			  $departureDate=$departureDateArr[0];
	 ...
	 $commissiondeff=round(($totalPaxFare+$totalTax)-$OfferedFare);
			...
			$duration=intdiv($durationmain, 60).'H :'. ($durationmain % 60).' M'; 
				...
				$namevalue ='UID="",TID="",ResultIndex="'.$ResultIndex.'",ORG_CODE="'.$todestexp[0].'",apiType="tripjack",ORG_NAME="'.$ORG_NAME.'",DES_CODE="'.$fromdestexp[0].'",DES_NAME="'.$DEST_NAME.'",DEP_DATE="'.$departureDate.'",DEP_TIME="'.$departurTime.'",ARRV_DATE="'.$arrivalDate.'",ARRV_TIME="'.$arrivalTime.'",FLIGHT_CODE="'.trim($FLIGHT_CODE).'",FLIGHT_NAME="'.$FLIGHT_NAME.'",FLIGHT_NO="'.$FLIGHT_NO.'",FARE_TYPE="'.$FARE_TYPE.'",SEAT="'.$seatRemaining.'",STOP="'.$totalStop.'",AMT="'.$getCalCost2[1].'",DISPLAY_AMT="'.$PublishedFare.'",DUR="'.($duration).'",S_CODE="'.$S_CODE.'",CN_CODE="'.$CN_CODE.'",OI="",PCC="'.$FareBasisCode.'",TAX_BREAKUP="0",FLIGHT_INFO="'.$FLIGHT_INFO.'",NET_FARE="'.$OfferedFare.'",refundyes="'.$isRefundable.'",AirlineRemark="",F_CLASS="'.$PC.'",SECTOR="'.$SECTOR.'",adfare="'.$adfare.'",agfare="'.$agfare.'",clfare="'.$clfare.'",CON_DETAILS="",searchJson="'.addslashes(serialize($searchJson)).'",agentId="'.$_SESSION['agentUserid'].'",tripType=2,acom="'.$commissiondeff.'",IsLCC="'.$IsLCC.'",netFareBeforecomm="'.($getCalCost2[1]-$commissiondeff).'",agentMarkup="'.$getAgentTaxonly[0].'",adminMarkup="'.$adminMarkup.'",totalTax="'.$totaldisplayTax.'",roundTripFlight=1,agentFixedMakup="'.$agentFixedMakup.'",uniqueSessionId="'.$_SESSION['uniqueSessionId'].'",uniqueId="'.$uniqueId.'" ';
