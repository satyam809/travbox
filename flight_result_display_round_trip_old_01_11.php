...
$mainlistcount=1;
$flightinfo=null;
$a=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'"  and uniqueSessionId="'.$_SESSION['uniqueSessionId'].'" and roundTripFlight=0 group by FLIGHT_NAME,FLIGHT_NO,DEP_TIME,uniqueId order by AMT asc');
...
$ns=1;
$farecolor='';
$b=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and FLIGHT_NAME="'.$res['FLIGHT_NAME'].'" and FLIGHT_NO="'.$res['FLIGHT_NO'].'" and DEP_TIME="'.$res['DEP_TIME'].'" group by PCC order by AMT asc');
...
while($res=mysqli_fetch_array($a)){
  ...
  $ab=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and uniqueSessionId="'.$_SESSION['uniqueSessionId'].'" and roundTripFlight=0 and FLIGHT_NAME="'.stripslashes($res['name']).'"  group by FLIGHT_NO,uniqueId order by AMT asc');
...
$abc=GetPageRecord('FLIGHT_NO,DEP_TIME,FLIGHT_CODE','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and uniqueSessionId="'.$_SESSION['uniqueSessionId'].'" and roundTripFlight=0 and FLIGHT_NAME="'.stripslashes($res['name']).'"    group by FLIGHT_NO,uniqueId order by AMT asc');
...
while($res=mysqli_fetch_array($a)){
  $flight=0;
  $flightinfo=null;
$ab=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and uniqueSessionId="'.$_SESSION['uniqueSessionId'].'" and roundTripFlight=1 and FLIGHT_NAME="'.stripslashes($res['name']).'" group by FLIGHT_NO,uniqueId order by AMT asc');
while($flightdata=mysqli_fetch_array($ab)){
  if(is_null($flightinfo)){
    $flightinfo=array("flightno" => $flightdata['FLIGHT_NO'],"flightcode" => $flightdata['FLIGHT_CODE'],"deptime" => $flightdata['DEP_TIME'],"arrtime" => $flightdata['ARRV_TIME']);
  }else{
...
$abc=GetPageRecord('FLIGHT_NO,DEP_TIME,FLIGHT_CODE','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and roundTripFlight=1 and FLIGHT_NAME="'.stripslashes($res['name']).'" group by FLIGHT_NO,uniqueId order by AMT asc');
$resf=mysqli_fetch_array($abc);
