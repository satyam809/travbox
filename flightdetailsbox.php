...
<?php if($layoverFlight['cT']>0){ ?>
  ...
  <div style="text-align: center; color: #0b0b0b; padding: 5px; background-color: #e4f8ff; font-weight: 600; margin:15px 0px;   ">Layover time: <?php echo $hours = intdiv($layoverFlight['cT'], 60).':'. ($layoverFlight['cT'] % 60); ?> hours</div>
...
$a=GetPageRecord('*','agent_fareTypedomesticFlightsMarkupMaster',' sectorType="'.$_SESSION['domesticorinter'].'"  and agentTypeGroupId="'.$admincommisiontype['commissionType'].'" and flightId in (select id from domesticFlightsMarkupMaster where name="'.$res['FLIGHT_NAME'].'")');   
...
$res1=mysqli_fetch_array($a);

$basefare = explode ("=", $str_arr[0]);

// echo $basefare[1];

if($res1['markupType']=='Flat'){

  $cancellationmarkup=$res1['cancellationMarkupValue'];

}else{

  $cancellationmarkup=($basefare[1]*$res1['cancellationMarkupValue']/100);
   ...
   if(!empty($fareRuleResultArr)){

   $defultCanAmt=$fareRuleResultArr['CANCELLATION']['DEFAULT']['amount'];

   $additionalCanFee=$fareRuleResultArr['CANCELLATION']['DEFAULT']['additionalFee'];

  $cancellationPolicyInfo=$fareRuleResultArr['CANCELLATION']['DEFAULT']['policyInfo'];

   $defultDateChangeAmt=$fareRuleResultArr['DATECHANGE']['DEFAULT']['amount'];

   $defultDeateChangeAddtAmt=$fareRuleResultArr['DATECHANGE']['DEFAULT']['additionalFee'];

  $deateChangePolcyInfo=$fareRuleResultArr['DATECHANGE']['DEFAULT']['policyInfo'];

   $noShowPolcyInfo=$fareRuleResultArr['NO_SHOW']['DEFAULT']['policyInfo'];

   $seatChargablePolcyInfo=$fareRuleResultArr['SEAT_CHARGEABLE']['DEFAULT']['policyInfo'];
...
<?php if(!empty($defultCanAmt) || !empty($additionalCanFee) || !empty($cancellationmarkup)) { ?>

<div style="font-size:18px; font-weight:600; margin-bottom:5px;">Cancellation Fee</div>

<div style="margin-bottom:2px; font-size:15px; font-weight:600; color:#CC0000;"><?php if(!empty($defultCanAmt)){ ?>&#8377; <?php echo str_replace('	','',str_replace('__nls__','',$defultCanAmt)); ?> 

 <?php if(!empty($defultCanAmt) && !empty($additionalCanFee) || !empty($cancellationmarkup)) { ?> + <?php  } ?>



   <?php } if(!empty($additionalCanFee) || !empty($cancellationmarkup)){ ?>  &#8377;<?= $additionalCanFee != 0 ? $additionalCanFee :  $cancellationmarkup; ?><?php } ?></div>



<div style="margin-bottom:20px;"><?php echo  strip_tags(str_replace('	','',str_replace('__nls__','',$cancellationPolicyInfo))); ?></div>

<?php } ?>

<input type="hidden" class="cancellationmarkup" value="<?= $cancellationmarkup;  ?>" name="cancellationmarkup">

<?php if(!empty($defultDateChangeAmt) || !empty($defultDeateChangeAddtAmt)){ ?>

<div style="font-size:18px; font-weight:600; margin-bottom:5px;">Date Change Fee</div>

<div style="margin-bottom:2px; font-size:15px; font-weight:600; color:#CC0000;"><?php if(!empty($defultDateChangeAmt)){ ?>&#8377; <?php echo str_replace('	','',str_replace('__nls__','',$defultDateChangeAmt)); } ?> 

<?php if(!empty($defultDateChangeAmt) && !empty($defultDeateChangeAddtAmt)){ echo '+'; } if(!empty($defultDeateChangeAddtAmt)){ ?>

  &#8377;<?php echo $defultDeateChangeAddtAmt; ?><?php } ?></div>

<div style="margin-bottom:20px;"><?php echo  strip_tags(str_replace('	','',str_replace('__nls__','',$deateChangePolcyInfo))); ?></div>
...
<!-- <div style="font-size:18px; font-weight:600; margin-bottom:5px;">No Show</div> -->
 ...
 if(!empty($noShowPolcyInfo)){ ?>



<div style="margin-bottom:20px;"><?php echo  strip_tags(str_replace('	','',str_replace('__nls__','',$noShowPolcyInfo))); ?></div>

<?php }

 if(!empty($seatChargablePolcyInfo)){ ?>
...
<div style="margin-bottom:20px;"><?php echo  strip_tags(str_replace('	','',str_replace('__nls__','',$seatChargablePolcyInfo))); ?></div>
